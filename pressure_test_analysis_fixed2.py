...
def evaluate_fold_change_accuracy(reference_fold_changes, database_fold_changes):
    ...
    for gene in key_genes:
        for model in models:
            if relative_errors[gene][model] != float('inf'):
                error_df.loc[gene, model] = float(relative_errors[gene][model])
            else:
                error_df.loc[gene, model] = np.nan
    
    plt.figure(figsize=(12, 10))
    sns.heatmap(error_df.astype(float), annot=True, cmap='YlOrRd', fmt='.2f', cbar_kws={'label': 'Relative Error'})
